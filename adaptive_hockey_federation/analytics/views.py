...
class AnalyticsListView(
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["dashboard"] = {
            ...
            "nosology": [
                (f"{nosology.name}", self.get_queryset().filter(
                    diagnosis__nosology=nosology).count())
                for i, nosology in enumerate(
                    Nosology.objects.filter(
                        diagnosis__in=self.get_queryset().values_list(
                            'diagnosis', flat=True).distinct()).distinct())
            ],
        }
        return context
