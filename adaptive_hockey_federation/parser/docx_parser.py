...
def read_file_columns(file: docx) -> list[docx]:
    ...
    """
    return [
        column for table in file.tables for index, column in enumerate(table.columns)
    ]
...
def read_file_text(file: docx) -> list[str]:
    ...
    """
    return [run.text for paragraph in file.paragraphs for run in paragraph.runs]


def get_counter_for_columns_parser(columns: list[docx]) -> int:
    count = 0
...
def columns_parser(
    ...
    output = [
        ...
        for column in columns
        if re.search(regular_expression, list(cell.text for cell in column.cells)[0])
...
def find_names(columns: list[docx], regular_expression: str) -> list[str]:
    ...
    return [name.split()[1].rstrip() for name in names_list if name]
...
def find_surnames(columns: list[docx], regular_expression: str) -> list[str]:
    ...
    return [surname.split()[0].rstrip() for surname in surnames_list if surname]
...
def find_dates_of_birth(
    ...
    for date_of_birth in dates_of_birth_list:
        if date_of_birth:
            try:
                for day, month, year in [re.sub(r"\D", " ", date_of_birth).split()]:
                    if len(year) == 2:
                        if int(year) > 23:
                            year = "19" + year
                        else:
                                            ...
                                            if text_clear[index + 3] == "Атал"
                                                ...
                                                if text_clear[index + 2] == "мечты"
                                                    ...
                                                    if text_clear[index + 1] == "Огни"
                                                        ...
                                                        "Энергия Жизни Краснодар"
                                                        if text_clear[index + 3]
                                                        == "Краснодар"
                                                        else (
                                                            "Энергия Жизни Сочи"
                                                            if text_clear[index + 4]
                                                            == "Сочи"
                                                            else (
                                                                "Динамо-Москва"
                                                                if text_clear[index + 1]
                                                                == "Динамо"
                                                                else (
                                                                    "Крылья Советов"
                                                                    if text_clear[
...
def find_positions(columns: list[docx], regular_expression: str) -> list[str]:
    ...
    return [
        (
            "нападающий"
            if re.search(
                r"^н|^Н|^H|^Нп|^нл|^нп|^цн|^лн|^Нап|^№|^А,|^К,", position.lstrip()
            )
            else (
                "защитник"
            ...
            if len(txt.split()) <= 4:
                try:
                    ...
                    if numeric_status:
                        ...
                        numeric_statuses_list.append(txt.split()[:3] + [numeric_status])
                except IndexError:
                    pass
...
def find_players_is_captain(columns: list[docx], regular_expression: str) -> list[bool]:
...
def find_discipline_level(
    ...
    for discipline_level in columns_parser(columns, regular_expression):
        if discipline_level:
            try:
                discipline_level = discipline_level.replace("Класс ", "").replace(
                    "класс ", ""
                )
...
def docx_parser(path: str, numeric_statuses: list[list[str]]) -> list[BaseUserInfo]:
    ...
    return [
        BaseUserInfo(
            ...
            is_assistant=(
                is_assistents[index]
                if is_assistents[index]
                else is_assistents_alt[index]
            ),
            is_captain=(
                is_captains[index] if is_captains[index] else is_captains_alt[index]
            ),
