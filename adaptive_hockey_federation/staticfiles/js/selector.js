class Selector {
  ...
  renderSelectedItems() {
    this.selectedList.innerHTML = '' // Очистим список перед рендерингом
    this.selectedItems.forEach(item => {
      if (
        ...
        li.classList.add('selector-item')
        li.textContent = item
        li.addEventListener('click', () => this.toggleSelection(item)) // Добавим обработчик для снятия выбора
  ...
  setupEventListeners() {
    ...
    this.searchAvailableInput.addEventListener('input', () =>
      this.renderAvailableItems()
    ) // Добавляем слушатель ввода для поиска в доступных элементах
    this.searchSelectInput.addEventListener('input', () =>
      this.renderSelectedItems()
    ) // Добавляем слушатель ввода для поиска в выбранных элементах
  }
