...
from main.models import GameDataPlayer
...
def bulk_create_gamedataplayer_objects(sender=None, **kwargs):
    ...
    if serializer.is_valid():
        ...
        game_id = object_data["game_id"]
        tracking = object_data["tracking"]
        with transaction.atomic():
            for track in tracking:
                player = track.get("player_id")
                del track["player_id"]
                track["game_id"] = game_id
                ...
                create_player_video.apply_async(
                    args=["Обработка с низки приоритетом"],
                    queue="slice_player_video_queue",
                    priority=255,
                )
    # Функция ничего не возвращает, в логах воркера она тоже не отслеживается.
