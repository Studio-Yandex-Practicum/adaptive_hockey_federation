...
8. [Имитация сервера DS (временный раздел)](#ds)
...
В составе команды `make run` также запускается пользовательский интерфейс по отслеживанию за работой воркеров.
...
# 6. Имитация сервера DS (временный раздел) <a id="ds"></a>
