...
#### Возможен запуск Pre-commit вручную, без коммита:
...
cd  adaptive_hockey_federation
```
2. Для быстрого развёртывания проекта воспользуйтесь командой:
```bash
make init-app
```
...
make run
```
4. Если в модели были внесены изменения воспользуйтесь командой:
```bash
make makemigrations
```
...
Файлы документов для парсинга хранятся в директории ``` /<корневая директория>/adaptive_hockey_federation/resourses/```
...
```
Результат парсинга сохранится в директории ```/<корневая директория>/adaptive_hockey_federation/resourses/``` в фале ```result.txt```.
Опциональные параметры:
-r, --result - Вывод в консоль извлеченных данных и статистики
